...
class Batch:
    ...
    metadata: Metadata
    flow_vars: Optional[Dict[str, torch.Tensor]] = None
    ...
    def normalise(self, surf_stats: dict[str, tuple[float, float]]) -> "Batch":
        ...
        # --- Create a dictionary for normalised surface variables ---
        normalised_surf_vars = {}
        for k, v in self.surf_vars.items():
            if k == 'wind_prompt':
                # For 'wind_prompt', skip normalisation as it's already in [0, 1] range
                normalised_surf_vars[k] = v 
            else:
                # For all other variables, apply normalisation
                normalised_surf_vars[k] = normalise_surf_var(v, k, stats=surf_stats)
        # ---
        
        return_data = Batch(
            surf_vars=normalised_surf_vars, # Use the new dictionary
            static_vars={
        ...
        if self.flow_vars is None: return return_data
        else:
            return_data.flow_vars = self.flow_vars
            return return_data
    ...
    def unnormalise(self, surf_stats: dict[str, tuple[float, float]]) -> "Batch":
        ...
        """
        unnormalised_surf_vars = {}
        for k, v in self.surf_vars.items():
            if k == 'wind_prompt':
                # Skip unnormalisation for 'wind_prompt'
                unnormalised_surf_vars[k] = v
            else:
                unnormalised_surf_vars[k] = unnormalise_surf_var(v, k, stats=surf_stats)
        
        return_data = Batch(
            surf_vars=unnormalised_surf_vars,
            static_vars={
    ...
    def crop(self, patch_size: int) -> "Batch":
        ...
        if h % patch_size == 0:
            ...
            if self.flow_vars is None: return return_data
                ...
                return_data.flow_vars = {k: v[..., :-1, :] for k, v in self.flow_vars.items()}
                return return_data
    ...
    def _fmap(self, f: Callable[[torch.Tensor], torch.Tensor]) -> "Batch":
        ...
        if self.flow_vars is None: return return_data
            ...
            return_data.flow_vars = {k: f(v) for k, v in self.flow_vars.items()}
            return return_data
    ...
    def regrid(self, res: float) -> "Batch":
        ...
        if self.flow_vars is None: return return_data
            ...
            return_data.flow_vars = {k: interpolate_res(v) for k, v in self.flow_vars.items()}
            return return_data

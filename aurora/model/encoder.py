...
class Perceiver3DEncoder(nn.Module):
    ...
    def __init__(
        ...
        use_flow: bool = False,
        flow_vars: Optional[tuple[str, ...]] = None,
    ) -> None:
        """Initialise.
        ...
        if self.use_flow:
            pass
            # self.flow_token_embeds = LevelPatchEmbed((flow_vars[0], flow_vars[2]), patch_size, embed_dim, max_history_size)
            # self.flow_level_encoding = nn.Parameter(torch.randn(embed_dim))
            # self.flow_mlp = MLP(embed_dim, int(embed_dim * mlp_ratio), dropout=drop_rate)
            # self.flow_norm = nn.LayerNorm(embed_dim)

        if (not self.level_condition) and len(atmos_vars) == 0:
            self.atmos_token_embeds = None
        elif not self.level_condition:
    ...
    def forward(self, batch: Batch, lead_time: timedelta) -> torch.Tensor:
        ...
        has_static = len(static_vars) > 0
        has_atmos  = len(atmos_vars) > 0
        if self.use_flow:
            pass
            # flow_vars = tuple(batch.flow_vars.keys() - ['maskpm2p5', 'maskpm10'])
            # has_flow  = len(flow_vars) > 0

        x_surf = torch.stack(tuple(batch.surf_vars.values()), dim=2)
        x_static = (
            torch.stack(tuple(batch.static_vars.values()), dim=2) if has_static else None
        )
        x_atmos = (
            torch.stack(tuple(batch.atmos_vars.values()), dim=2) if has_atmos else None
        )

        if self.use_flow:
            pass
            # x_flow = torch.stack(tuple(batch.flow_vars[key] for key in flow_vars), dim=2)
            # x_flow = torch.stack([x_flow[:, 0, :2], x_flow[:, 0, 2:]], dim=2)
        ...
        if not has_static:
            ...
            if self.dynamic_vars:
                ...
                if has_static:
                    ...
                    x_surf = torch.cat((x_surf, x_dynamic), dim=2)
                    surf_vars = surf_vars + dynamic_vars
        ...
        # print(f"lat: {lat}, lon: {lon}, H: {H}, W: {W}")
        ...
        if self.use_flow:
            pass
            # x_flow = rearrange(x_flow, "b t v h w -> b v t h w")
            # x_flow = self.flow_token_embeds(x_flow, (self.flow_vars[0], self.flow_vars[2]))  # (B, L, D)
            # ftype = x_flow.dtype
        ...
        if has_atmos:
            if not self.level_condition:
                ...
                # --------------------------------------------------------------
                # 1. Collapse batch × pressure‑levels so that PatchEmbed treats
                ...
                # --------------------------------------------------------------
                # 2. Undo the collapse.  *c_levels* must be the number of
                #    pressure levels, **not** the number of surface variables.
                #    Inferring the value from the tensor keeps the code valid
                #    for
                #      · OBS‑only   (1 level, dummy variable)
                #      · CMAQ‑only  (5 levels: sfc + 925 + 850 + 700 + 500 hPa)
                #      · OBS + CMAQ (same 5 levels)
                # --------------------------------------------------------------
                c_levels = x_atmos.shape[0] // B          # safe for any dataset
                if c_levels * B != x_atmos.shape[0]:      # guard against bugs
                    raise RuntimeError(
                        f"Inconsistent atmos token shape: first dim "
                        f"{x_atmos.shape[0]} is not divisible by batch size {B}"
                    )
                x_atmos = rearrange(
                    x_atmos, "(b c) l d -> b c l d", b=B, c=c_levels
                )
            else:
        ...
        if self.use_flow:
            pass
            # x_flow = x_flow + self.flow_level_encoding[None, None, :].to(dtype=ftype)
            # x_flow = x_flow + self.flow_norm(self.flow_mlp(x_flow))
            # x = torch.cat((x, x_flow.unsqueeze(1)), dim=1)

...
class ResBlock(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.n1 = _group_norm(out_ch)
        self.c2 = nn.Conv2d(out_ch, out_ch, 3, padding=1)
        self.n2 = _group_norm(out_ch)
...
class Attention2D(nn.Module):
    """Lightweight self-attention over HxW tokens."""
    def __init__(self, channels: int, heads: int = 4, dropout: float = 0.0):
        super().__init__()
        self.pre  = nn.Conv2d(channels, channels, 1)
        self.attn = nn.MultiheadAttention(channels, heads, dropout=dropout, batch_first=False)
        self.post = nn.Conv2d(channels, channels, 1)

    def forward(self, x):
        B, C, H, W = x.shape
        h = self.pre(x)                                # B,C,H,W
        tokens = h.flatten(2).permute(2, 0, 1)         # S,B,C
        out, _ = self.attn(tokens, tokens, tokens, need_weights=False)
        out = out.permute(1, 2, 0).reshape(B, C, H, W) # B,C,H,W
        return self.post(out)

# ------------------ physics: advection ------------------
def advect_scalar_semi_lagrangian(u_t: torch.Tensor, v: torch.Tensor, dt: float) -> torch.Tensor:
    """
    u_{t+1}(x) = u_t(x - dt * v(x))  using bilinear sampling (stable).
    u_t: (B,1,H,W), v: (B,2,H,W) with units 'pixels per step' (scale if needed).
    """
    B, _, H, W = u_t.shape
    device = u_t.device
    ...
    grid0 = torch.stack([xx, yy], dim=-1).unsqueeze(0).repeat(B, 1, 1, 1)  # (B,H,W,2)

    vx, vy = v[:, 0], v[:, 1]
    vxn = 2.0 * vx / max(W - 1, 1)
    vyn = 2.0 * vy / max(H - 1, 1)
    flow = torch.stack([vxn, vyn], dim=-1)  # (B,H,W,2)

    grid = grid0 - dt * flow
    return F.grid_sample(u_t, grid, mode="bilinear", padding_mode="border", align_corners=True)

# ------------------ model pieces ------------------
class VelocityUpdate(nn.Module):
    """
    delta_v = Conv(u,v) + Attn(u,v)
    Inputs:  u_t (B,1,H,W), v_t (B,2,H,W)
    Output:  delta_v (B,2,H,W)
    """
    def __init__(self, base: int = 64, heads: int = 4):
        super().__init__()
        in_ch = 1 + 2
        self.stem = nn.Sequential(nn.Conv2d(in_ch, base, 3, padding=1), _group_norm(base), nn.GELU())

        self.conv_path = nn.Sequential(ResBlock(base, base), ResBlock(base, base), nn.Conv2d(base, 2, 3, padding=1))
        self.attn_path = nn.Sequential(ResBlock(base, base), Attention2D(base, heads=heads), nn.Conv2d(base, 2, 1))

    def forward(self, u_t, v_t):
        h = self.stem(torch.cat([u_t, v_t], dim=1))
        return self.conv_path(h) + self.attn_path(h)

class UFromUV(nn.Module):
    """
    û_{t+1} (neural) from concat[u_t, v_{t+1}] via Conv+Attention
    Input : u_t (B,1,H,W), v_{t+1} (B,2,H,W)
    Output: u_hat (B,1,H,W)
    """
    def __init__(self, base: int = 64, heads: int = 4):
        super().__init__()
        in_ch = 1 + 2
        self.stem = nn.Sequential(nn.Conv2d(in_ch, base, 3, padding=1), _group_norm(base), nn.GELU())
        self.conv_path = nn.Sequential(ResBlock(base, base), ResBlock(base, base), nn.Conv2d(base, 1, 3, padding=1))
        self.attn_path = nn.Sequential(ResBlock(base, base), Attention2D(base, heads=heads), nn.Conv2d(base, 1, 1))

    def forward(self, u_t, v_next):
        h = self.stem(torch.cat([u_t, v_next], dim=1))
        return self.conv_path(h) + self.attn_path(h)

class RefineFuse(nn.Module):
    """
    Refine final u using two predictions: concat[u_hat, u_phys] → Conv+Attn → u_final
    """
    def __init__(self, base: int = 64, heads: int = 4):
        super().__init__()
        in_ch = 3  # u_hat + u_phys
        self.stem = nn.Sequential(nn.Conv2d(in_ch, base, 3, padding=1), _group_norm(base), nn.GELU())
        self.core = nn.Sequential(
            ResBlock(base, base),
            Attention2D(base, heads=heads),
            ResBlock(base, base),
        )
        self.out = nn.Conv2d(base, 1, 3, padding=1)

    def forward(self, u_hat, u_phys):
        h = self.stem(torch.cat([u_hat, u_phys], dim=1))
        h = self.core(h)
        return self.out(h)
...
class Aurora(torch.nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        use_flow: bool = False,
        flow_vars: tuple[str, ...] = ("flowpm2p5x", "flowpm2p5y", "flowpm10x", "flowpm10y"),
    ) -> None:
        ...
        if (not atmos_vars) or latent_levels == 1:
        ...
        if use_flow:
            self.f_upd = VelocityUpdate(base=32, heads=4)
            self.u_from  = UFromUV(base=32, heads=4)
            self.refiner = RefineFuse(base=32, heads=4)
    ...
    def forward(self, batch: Batch) -> Batch:
        ...
        if self.use_flow:
            batch = dataclasses.replace(
                ...
                flow_vars={k: v for k, v in batch.flow_vars.items()}
            )
        ...
        # if self.use_flow:
        #     for key in transformed_batch.surf_vars.keys():
        #         print(transformed_batch.surf_vars[key].shape) # torch.Size([16, 2, 128, 174])
        #     for key in transformed_batch.flow_vars.keys():
        #         print(transformed_batch.flow_vars[key].shape) # torch.Size([16, 1, 128, 174])
        #     raise 0
        ...
        tokens_per_patch = (H // self.encoder.patch_size) * (W // self.encoder.patch_size)
        c_levels = x.shape[1] // tokens_per_patch          
        patch_res = (c_levels, H // self.encoder.patch_size, W // self.encoder.patch_size)
        ...
        if self.use_flow:
            pred = dataclasses.replace(
                pred,
                flow_vars={k: v[:, None] for k, v in batch.flow_vars.items()},
            )

            u25_t = batch.surf_vars['pm2p5_cmaq'][:, -1][:, None]
            u25_t2 = pred.surf_vars['pm2p5'][:, -1][:, None].clone().detach()
            v25_tx = batch.flow_vars['flowpm2p5x']
            v25_ty = batch.flow_vars['flowpm2p5y']
            v25_t = torch.cat([v25_tx, v25_ty], dim=1)#*10

            delta_v25 = self.f_upd(u25_t, v25_t)          # 1) neural update for velocity
            v25_next  = v25_t + delta_v25                 #    residual update (dt absorbed)

            # u25_phys  = advect_scalar_semi_lagrangian(u25_t, v25_next, dt=1.0)  # 3) physical u_{t+1}
            v25_nextx = v25_next[:, 0][:, None]# / 10
            v25_nexty = v25_next[:, 1][:, None]# / 10

            u25_final = self.refiner(u25_t2, v25_next)

            u10_t = batch.surf_vars['pm10_cmaq'][:, -1][:, None]
            u10_t2 = pred.surf_vars['pm10'][:, -1][:, None].clone().detach()
            v10_tx = batch.flow_vars['flowpm10x']
            v10_ty = batch.flow_vars['flowpm10y']
            v10_t = torch.cat([v10_tx, v10_ty], dim=1)#*10

            delta_v10 = self.f_upd(u10_t, v10_t)          # 1) neural update for velocity
            v10_next  = v10_t + delta_v10                 #    residual update (dt absorbed)

            # u10_phys  = advect_scalar_semi_lagrangian(u10_t, v10_next, dt=1.0)  # 3) physical u_{t+1}
            v10_nextx = v10_next[:, 0][:, None]# / 10
            v10_nexty = v10_next[:, 1][:, None]# / 10

            u10_final = self.refiner(u10_t2, v10_next)

            pred.surf_vars['pm2p5_origin'] = pred.surf_vars['pm2p5']
            pred.surf_vars['pm10_origin'] = pred.surf_vars['pm10']
            pred.surf_vars['pm2p5'] = u25_final
            pred.surf_vars['pm10'] = u10_final
            pred.flow_vars['flowpm2p5x'] = v25_nextx
            pred.flow_vars['flowpm2p5y'] = v25_nexty
            pred.flow_vars['flowpm10x'] = v10_nextx
            pred.flow_vars['flowpm10y'] = v10_nexty

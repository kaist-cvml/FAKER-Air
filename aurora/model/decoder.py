...
class Perceiver3DDecoder(nn.Module):
    ...
    def __init__(
        ...
        use_flow: bool = False,
        flow_vars: tuple[str, ...] = ("flowpm2p5", "flowpm10"),
    ) -> None:
        """Initialise.
        ...
        if self.use_flow:
            pass
            # self.flow_heads = nn.ParameterDict(
            #     {name: LinearPatchReconstruction(embed_dim, patch_size**2) for name in flow_vars}
            # )
    ...
    def forward(
        ...
        if len(atmos_vars) > 0: 
            ...
            atmos_preds_dict = {v: atmos_preds[:, i] for i, v in enumerate(atmos_vars)}
        ...
        if self.use_flow:
            ...
            x_flow = torch.stack([self.flow_heads[name](x[..., :1, :]) for name in flow_vars], dim=-1)
            x_flow = x_flow.reshape(*x_flow.shape[:3], -1)  # (B, L, 1, V_S*p*p)
            flow_preds = unpatchify(x_flow, len(flow_vars), H, W, self.patch_size)
            flow_preds = flow_preds.squeeze(2)  # (B, V_S, H, W)
            ...
            return Batch(
                ...
                {v: flow_preds[:, i] for i, v in enumerate(flow_vars)},
            )
